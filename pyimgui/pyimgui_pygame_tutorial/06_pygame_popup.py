...
def main():
    ...
    # Set screen title
    pygame.display.set_caption("Pyimgui: 6_popup")
    ...
    while 1:
        ...
        if imgui.button("Button 1"):
            # ... with an attached popup. The label "button-popup" is only
            # an identifier which points to the defined popup content (see below).
            # The popup itself can only be activated via
            # an element with function/action (e.g. a button).
            imgui.open_popup("button-popup")

        imgui.same_line()

        # Create a second button with the same attached popup
        if imgui.button("Button 2"):
            imgui.open_popup("button-popup")

        # The definition of the popup context
        # The popup can e attached to any number of elements.
        if imgui.begin_popup("button-popup"):  # identifier as string
            # Content of the popup
            imgui.text("Push the button")
            imgui.separator()
            imgui.selectable("One")
            imgui.selectable("Two")
            imgui.selectable("Three")
            # Closure of the popup definition
            imgui.end_popup()
